...
export class MqttBrokerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly sensorTopic = MQTT_SENSOR_TOPIC;
  private readonly commandTopic = MQTT_COMMAND_TOPIC;
  private readonly ackTopic = MQTT_ACK_TOPIC;
  private readonly statusTopic = MQTT_STATUS_TOPIC;
  ...
  private bootstrapClient(): void {
    ...
    this.client.on('connect', () => {
      ...
      this.subscribe([this.sensorTopic, this.commandTopic, this.ackTopic, this.statusTopic]);
    });
  ...
  getStatus(): { connected: boolean; brokerUrl: string; sensorTopic: string; commandTopic: string; ackTopic: string; statusTopic: string } {
    return {
